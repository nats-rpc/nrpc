...
// {{$typesPrefix}}{{.GetName}}Server is the interface that providers of the service
// {{.GetName}} should implement.
type {{$typesPrefix}}{{.GetName}}Server interface {
	{{- range .Method}}
...
// subscription using a given {{$typesPrefix}}{{.GetName}}Server implementation.
type {{.GetName}}Handler struct {
	ctx     context.Context
	workers *nrpc.WorkerPool
	nc      nrpc.NatsConn
	server  {{$typesPrefix}}{{.GetName}}Server

	encodings []string
}

func New{{.GetName}}Handler(ctx context.Context, nc nrpc.NatsConn, s {{$typesPrefix}}{{.GetName}}Server) *{{.GetName}}Handler {
...
func New{{.GetName}}ConcurrentHandler(workers *nrpc.WorkerPool, nc nrpc.NatsConn, s {{$typesPrefix}}{{.GetName}}Server) *{{.GetName}}Handler {
	return &{{.GetName}}Handler{
		workers: workers,
		nc:      nc,
		server:  s,
...
type {{$typesPrefix}}{{.GetName}}Client struct {
	nc      nrpc.NatsConn
...
func New{{$typesPrefix}}{{.GetName}}Client(nc nrpc.NatsConn
	{{- range $pkgSubjectParams -}}
	, pkgParam{{.}} string
	{{- end -}}
	{{- range GetServiceSubjectParams . -}}
	, svcParam{{ . }} string
	{{- end -}}
	) *{{$typesPrefix}}{{.GetName}}Client {
	return &{{$typesPrefix}}{{.GetName}}Client{
		nc:      nc,
		{{- if ne 0 (len $pkgSubject)}}
...
{{- $serviceName := (print $typesPrefix .GetName)}}
{{- $serviceSubjectParams := GetServiceSubjectParams .}}
{{- range .Method}}
{{- $resultType := GetResultType .}}
{{- if eq .GetInputType ".nrpc.NoRequest"}}
...
type Client struct {
	...
	{{- range .Service}}
	{{.GetName}} *{{$typesPrefix}}{{.GetName}}Client
	{{- end}}
}
...
func NewClient(nc nrpc.NatsConn
	...
	c.{{.GetName}} = New{{$typesPrefix}}{{.GetName}}Client(nc
	{{- range $pkgSubjectParams -}}
		, c.pkgParam{{ . }}
	{{- end}})
	{{- end}}
...
func (c *Client) Set{{.GetName}}Params(
	...
	c.{{.GetName}} = New{{$typesPrefix}}{{.GetName}}Client(
		c.nc,
	{{- range $pkgSubjectParams}}
		c.pkgParam{{ . }},
	{{- end}}
...
func (c *Client) New{{$typesPrefix}}{{.GetName}}(
	{{- range GetServiceSubjectParams .}}
	{{ . }} string,
	{{- end}}
) *{{$typesPrefix}}{{.GetName}}Client {
	client := New{{$typesPrefix}}{{.GetName}}Client(
		c.nc,
	{{- range $pkgSubjectParams}}
		c.pkgParam{{ . }},
	{{- end}}
